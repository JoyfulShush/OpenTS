...
A node for the acted side's tower precedes every base-defense placeholder a plan carries where that side lists one in [`AIWallTowers`](/keys/aiwalltowers/#scope-side), and once the tower is placed the defense node behind it moves onto the tower's cell. A tower the house already owns is left out of the buildings [defense candidates](/systems/ai-base-building/#base-defenses) have their prerequisites tested against, but every type the acted side lists is put back into that list unconditionally, so a defense that names it as a prerequisite is buildable before any tower stands.

The value also seeds the first side's [`AIWallTowers`](/keys/aiwalltowers/#scope-side) as each rules file sets it, which is how the computer's GDI bases come to be ringed with towers; an `AIWallTowers=` in that side's own section of the same file overrides it. The base planner still compares a base-defense node it cannot fill against this type before deleting it, and a base taken over by the computer recognizes its wall towers by it, whatever side the house plays for.
